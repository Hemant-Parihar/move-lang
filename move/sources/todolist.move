module todolist_addr::todolist {
    ...
    // Error code
    const TODO_LIST_NOT_INITIALIZED: u64 = 1;
    const TASK_DOESNT_EXIST: u64 = 2;
    const TASK_IS_ALREADY_COMPLETED: u64 = 3;
    ...
    public entry fun create_task(account: &signer, content: String) acquires TodoList {
        ...
        assert!(exists<TodoList>(signer_address), TODO_LIST_NOT_INITIALIZED);
    ...
    public entry fun complete_task(account: &signer, task_id: u64) acquires TodoList {
        ...
        let todo_list = borrow_global_mut<TodoList>(signer_address);
        // assert task exists
        assert!(table::contains(&todo_list.tasks, task_id), TASK_DOESNT_EXIST);
        // gets the task matched the task_id
        let task_record = table::borrow_mut(&mut todo_list.tasks, task_id);
        // assert task is not completed
        assert!(task_record.completed == false, TASK_IS_ALREADY_COMPLETED);
    ...
    #[test(admin = @0x123)]
    #[expected_failure(abort_code = TODO_LIST_NOT_INITIALIZED)]
